...
void start_asset_watcher(NotNull<EditorContext *> ctx) {
  ...
  ScratchArena scratch;
  EditorProjectContext *project = ctx->m_project;
  project->m_root_watch_handle =
      CreateFileW(utf8_to_raw_path(scratch, project->m_directory.m_str),
                  GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                  FILE_ATTRIBUTE_NORMAL | FILE_FLAG_BACKUP_SEMANTICS, nullptr);
  if (project->m_root_watch_handle == INVALID_HANDLE_VALUE) {
    fmt::println(stderr, "Failed to open {} for change monitoring: {}",
                 project->m_directory, GetLastError());
  }
#endif
}
...
void run_asset_watcher(NotNull<EditorContext *> ctx) {
  ...
  alignas(DWORD) char buffer[2048];
  DWORD num_returned = 0;
  if (!ReadDirectoryChangesW(
          project->m_root_watch_handle, buffer, sizeof(buffer), false,
          FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_FILE_NAME |
              FILE_NOTIFY_CHANGE_LAST_WRITE,
          &num_returned, nullptr, nullptr)) {
    fmt::println(stderr, "ReadDirectoryChangesW failed: %d", GetLastError());
    return;
  }
  if (num_returned == 0) {
    goto reset;
  ...
  for (usize offset = 0; offset < num_returned;) {
    const auto *event = (const FILE_NOTIFY_INFORMATION *)&buffer[offset];
