...
bool EditableTreeNode(String8 name, NotNull<Arena *> arena,
  ...
  const char *c_str = name.zero_terminated(scratch);
  ImGuiID id = ImGui::GetID(c_str);

  ImGuiStorage *storage = ImGui::GetStateStorage();
  bool editing = storage->GetBool(id, false);

  if (ImGui::TreeNodeEx(c_str, tree_flags | ImGuiTreeNodeFlags_OpenOnArrow,
                        editing ? "" : c_str)) {
    ...
    if (is_node_hovered &&
      ...
      if (buf->m_capacity < len) {
        buf->reserve(arena, len + 1);
      }
      strcpy_s(buf->data(), buf->m_capacity, c_str);
    }
    ...
    if (editing) {
      ...
      if (InputText("##rename", arena, buf, input_flags)) {
        editing_done = true;
        editing = false;
      ...
      if (!isInputTextHovered && !is_node_hovered &&
