...
int main() {
  ...
  for (u32 i : range<u32>(1, NUM_CORPUT_BASE_2 + 1)) {
    fmt::println("{}: {}", i, glsl::corput_base_2(i));
  }
  ...
  for (u32 i : range<u32>(1, NUM_CORPUT_BASE_3 + 1)) {
    fmt::println("{}: {}", i, glsl::corput_base_3(i));
  }
  ...
  for (u32 i : range<u32>(1, NUM_HAMMERSLEY_2D + 1)) {
    glm::vec2 xy = glsl::hammersley_2d(i, NUM_HAMMERSLEY_2D);
  ...
  for (u32 i : range<u32>(1, NUM_HAMMERSLEY_3D + 1)) {
    glm::vec3 xyz = glsl::hammersley_3d(i, NUM_HAMMERSLEY_3D);
